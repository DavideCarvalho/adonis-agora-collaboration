...
4. **`listComments` filters by space when given one**, and returns every space when not. Sorting is
   ...
   (`{ page?, size? }` — a **1-based** page number and a page size, the same shape
   `@adonis-agora/filter` uses): omit it and return every row — the HTTP routes always supply one, so
