...
- [#68](https://github.com/DavideCarvalho/adonis-agora-collaboration/pull/68) [`38684f1`](https://github.com/DavideCarvalho/adonis-agora-collaboration/commit/38684f1d67af998d2df2cd91cbdda9c1558625e0) Thanks [@DavideCarvalho](https://github.com/DavideCarvalho)! - Fix the built-in routes against a real Adonis app, and the PartyKit driver against a real worker.
...
- [#66](https://github.com/DavideCarvalho/adonis-agora-collaboration/pull/66) [`ce73222`](https://github.com/DavideCarvalho/adonis-agora-collaboration/commit/ce73222746d9063021d7635d59616b6e15869d19) Thanks [@DavideCarvalho](https://github.com/DavideCarvalho)! - Add `beginAdmission` and `isEphemeralRoom` to the config (self-hosted Yjs engine), and forward every config key from the provider to the manager
  ...
  - `YjsDriver.close()` no longer hangs while a socket is still in its handshake: Hocuspocus' `closeConnections` only reaches admitted sockets, and `wss.close` waited for the rest forever. Remaining sockets are now terminated on shutdown.
...
- 7ebbdb9: **Breaking:** the list pagination shape is now `{ page, size }` instead of `{ limit, offset }`.
...
- aa33ea1: `GET /collaboration/versions` and `GET /collaboration/comments` no longer return every row.
