...
4. **`listComments` filters by space when given one**, and returns every space when not. Sorting is
   handled above you. Both `listVersions` and `listComments` accept an optional `page`
   (`{ limit?, offset? }`): omit it and return every row — the HTTP routes always supply one, so
   honour it or a long history comes back as one ever-growing array. `getComment` is **optional**: implement it when your backend can look one
...
export class S3CollabStorage implements CollaborationStorage {
  ...
  async listVersions(docName: string, page?: ListPageOptions): Promise<CollabVersion[]> {
    ...
    if (page) {
      query.limit(Math.min(Math.max(page.limit ?? 100, 1), 500)).offset(Math.max(page.offset ?? 0, 0))
    }
