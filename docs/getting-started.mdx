...
- **Type your document names.** `node ace make:collab-document researches/writing` generates the
  ...
  document name in the app into a union. If you declared your documents inline in
  `config/collaboration.ts` instead — as above — the registry has nothing to scan; derive the union
  from the config with `InferCollabDocumentNames<typeof collaborationConfig>`.
